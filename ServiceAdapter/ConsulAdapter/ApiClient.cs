...
namespace ServiceAdapter.ConsulAdapter
{
    public class ApiClient : IApiClient
    {
        ...
        /// <returns></returns>
        private string GetServiceName(ApiRequest data)
        {
            var apiStation = data.url.Split('/')[1];
            return apiStation;
        }

        private async Task<ApiResponse<ReturnType>> BeforeCallApi<ReturnType>(ApiRequest req)
        {
        ...
        /// <returns></returns>
        public ApiResponse<ReturnType> RequestApi<ReturnType>(ApiRequest req)
        {
            return RequestApiAsync<ReturnType>(req).Result;
        }
        ...
        /// <param name="req"></param>
        /// <returns></returns>
        public async Task<ApiResponse<ReturnType>> RequestApiAsync<ReturnType>(ApiRequest req)
        {
            //(x.0)BeforeCallApi
        ...
        private Type GetUnderlyingTypeIfNullable(Type type)
        {
        ...
        private bool TypeIsValueTypeOrStringType(Type type)
        {
            if (type == null)
            {
                return false;
        ...
        private T DeserializeFromString<T>(string value)
        {
