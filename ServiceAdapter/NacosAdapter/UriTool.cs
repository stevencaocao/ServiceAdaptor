...
namespace ServiceAdapter.NacosAdapter
{
    internal static class UriTool
    {
        ...
        private static string ReplaceAddress(string address, string preferredNetworks)
        {
            ...
            else if (address.ToLower().Contains("localhost"))//StringComparison.OrdinalIgnoreCase
            {
                address = address.ToLower().Replace("localhost", ip);// StringComparison.OrdinalIgnoreCase
            }
